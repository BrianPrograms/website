...
const launch=process.argv[2];assert(isValidDate(launch)&&process.argv.length===3,'Supply a temporary preview YYYY-MM-DD launch date');
const config=JSON.parse(readFileSync(path.join(root,'wrangler.jsonc'),'utf8'));
assert.equal(config.name,'website');assert(!config.vars?.MAKE10_LAUNCH_DATE,'Production date must remain unset');
...
const staging=mkdtempSync(path.join(root,'.wrangler','pages-preview-')),output=path.join(staging,'public');
...
config.env.preview.vars={...config.env.preview.vars,MAKE10_LAUNCH_DATE:launch};
...
writeFileSync(path.join(root,'.wrangler','make10-preview-build.json'),JSON.stringify({staging,output,launch,assets:assets.length},null,2)+'\n');
console.log(JSON.stringify({staging,output,launch,assets:assets.length},null,2));
